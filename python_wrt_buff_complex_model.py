...
### Use python to output SLiM file for running Buffering Complex models
...
def wrt2buff(buff_file, conserv_aa_ratio, sel_aa_ratio, neutral_cutoff, del_cutoff, conserv_coeff, sel_coeff, phase_len, neutral_burnIn_len, wolbachia_non_burnIn_len, wolbachia_first=True):
    """
        wrt2buff: 
        ...
        Arguments:
            ...
            neutral_cutoff (float): the cutoff for neutrality zone
            del_cutoff (float): the ms cutoff for deleterious mutations
    ...
    with open(buff_file, 'w') as f:
        ...
        f.write("\t\t\t// compare the mutated amino acid with the ancestral codon (true optimum)\n")
        f.write("\t\t\tanc_codon = nucleotidesToCodons(AncSeq)[mismatch_idx];\n")
        f.write("\t\t\t// get the Miyata distance between the translated aa's\n")
        f.write("\t\t\tdiff_index = mut_codon*64 + anc_codon;\n")
        f.write("\t\t\tms_mut = M[diff_index];\n")
        f.write("\t\t\t// if too diversifying: deleterious\n")
        f.write("\t\t\tif (ms_mut > {})\n".format(del_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
        f.write("\t\t\t// if relatively similar: neutral, sel coeff = 0 (by SLiM default, no need to change)\n")
        f.write("\t\t}\n")
        f.write("\t}\n")
        f.write("\treturn T;\n")
        f.write("}\n\n\n")
        ...
        f.write("\t\t\tif (ms_mut - ms_original > {})\n".format(neutral_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
        f.write("\t\t\t// if almost similar: neutral\n")
        f.write("\t\t\telse if ((ms_mut - ms_original >= -{0}) & (ms_mut - ms_original <= {0}))\n".format(neutral_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(0.0);\n")
        f.write("\t\t\t// if more similar compared with the original: beneficial\n")
        f.write("\t\t\telse\n")
        ...
        f.write("\t\t\tms_mut = M[diff_index];\n")
        if (wolbachia_first):
            f.write("\t\t\t// only punish the extremely diversified aa's\n")
            f.write("\t\t\tif (ms_mut > {})\n".format(del_cutoff))
            f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
            f.write("\t\t\t// otherwise just neutral\n")
            f.write("\t\t\telse \n")
            f.write("\t\t\t\tmut.setSelectionCoeff(0.0);\n")
        else:
            f.write("\t\t\t// if more diversifying compared with the original: deleterious\n")
        ...
        f.write("\t\t\t// if more diversifying compared with the original: deleterious\n")
        f.write("\t\t\tif (ms_mut - ms_ref > {})\n".format(neutral_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
        f.write("\t\t\t// if almost similar: neutral\n")
        f.write("\t\t\telse if ((ms_mut - ms_ref >= -{0}) & (ms_mut - ms_ref <= {0}))\n".format(neutral_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(0.0);\n")
        f.write("\t\t\t// if more similar compared with the original: beneficial\n")
        f.write("\t\t\telse\n")
        f.write("\t\t\t\tmut.setSelectionCoeff(BenSelCoeff);\n")
        ...
        f.write("\t\t\tmut_codon = mut_codons[mismatch_idx];\n")
        f.write("\t\t\tanc_codon = nucleotidesToCodons(AncSeq)[mismatch_idx];\n")
        f.write("\t\t\t// get the Miyata distance between the mutated aa and ANCESTRAL aa\n")
        f.write("\t\t\tdiff_index = mut_codon*64 + anc_codon;\n")
        f.write("\t\t\tmiyata_score = M[diff_index];\n")
        f.write("\t\t\t// if very diversifying: deleterious\n")
        f.write("\t\t\tif (miyata_score > {})\n".format(del_cutoff))
        f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
        f.write("\t\t\t// if not very diversifying: neutral\n")
        f.write("\t\t\telse\n")
        f.write("\t\t\t\tmut.setSelectionCoeff(0.0);\n")
        f.write("\t\t}\n")
        f.write("\t}\n")
...
def get_opts_snpCalling(parser):
    ...
    ## input the ratio of neutral cutoff
    parser.add_argument('--neutralCutoff', help='neutralCutoff', required=True)
    ## input the ratio of del cutoff
    parser.add_argument('--delCutoff', help='delCutoff', required=True)
...
#     wolbachia_first = bool(int(sys.argv[3]))
    ...
    neutral_cutoff = float(args.neutralCutoff)
    del_cutoff = float(args.delCutoff)
    
    print('SLiM Buff Neutrality Model with NeatCutoff={} DelCutoff={} ConservRatio={} SelRatio={} s={} phase={} Run{}'.format(neutral_cutoff, del_cutoff, conserv_aa_ratio, sel_aa_ratio, sel_coeff, phase_len, run_number))
    ...
    wrt2buff(buff_file+'.slim', conserv_aa_ratio, sel_aa_ratio, neutral_cutoff, del_cutoff, conserv_coeff, sel_coeff, phase_len, neutral_burnIn_len, wolbachia_non_burnIn_len, wolbachia_first)
