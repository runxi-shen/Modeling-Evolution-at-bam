...
### Use python to output SLiM file for running Conflict Complex models for fitting empirical data by varying RC and RS
...
def wrt2conf(conf_file, conserv_aa_ratio, sel_aa_ratio, conserv_coeff, pop_sample, sel_coeff, neutral_cutoff=1.0, del_cutoff=3.0, phase_len=12500, neutral_burnIn_len=20000, wolbachia_non_burnIn_len=25000, wolbachia_first=True):
    """
        wrt2conf: 
    ...
    with open(conf_file, 'w') as f:
        ...
        f.write("\t\t\t// if relatively diversifying: beneficial\n")
        f.write("\t\t\telse if ((ms_mut>{0}) & (ms_mut<={1}))\n".format(neutral_cutoff, del_cutoff))
        ...
        f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
        f.write("\t\t\t// if similar: neutral; unnessary but for clarity of codes still stated\n")
        f.write("\t\t\telse\n")
        f.write("\t\t\t\tmut.setSelectionCoeff(0.0);\n")
        f.write("\t\t}\n")
        f.write("\t}\n")
        ...
        if (wolbachia_first):
            ...
            f.write("\t\t\tif (mut_ms > {})\n".format(del_cutoff))
            f.write("\t\t\t\tmut.setSelectionCoeff(DelSelCoeff);\n")
            f.write("\t\t\t// if relatively more similar: beneficial\n")
            f.write("\t\t\telse if ((mut_ms>{0}) & (mut_ms<={1}))\n".format(neutral_cutoff, del_cutoff))
            f.write("\t\t\t\tmut.setSelectionCoeff(BenSelCoeff);\n")
        else:
            f.write("\t\t\t// if a little bit diversifying: deleterious\n")
            f.write("\t\t\tif (mut_ms > {})\n".format(neutral_cutoff))
        ...
        f.write("\t\t// get the mismatch codon in the two sequences and check it's NOT conserved \n")
...
#         if (wolbachia_first):
        f.write("\t\t\tg.outputVCF('slim_bam_cds_conf_s{:.2e}_'+'Gen'+sim.generation+'.vcf', simplifyNucleotides=F);\n".format(sel_coeff))
#         else:
...
#     wolbachia_first = bool(int(sys.argv[3]))
    ...
    print('SLiM Conf Neutrality with Constraints Model with NeatCutoff={} DelCutoff={} ConservRatio={} SelRatio={} s={} phase={} Run{}'.format(neutral_cutoff, del_cutoff, conserv_aa_ratio, sel_aa_ratio, sel_coeff, phase_len, run_number))
    ...
    wrt2conf(conf_file+'.slim', conserv_aa_ratio, sel_aa_ratio, conserv_coeff, pop_sample, sel_coeff)
    ...
    vcf_file = 'slim_bam_cds_conf_s{:.2e}_Gen{}.vcf'.format(sel_coeff, neutral_burnIn_len+wolbachia_non_burnIn_len)
    fasta_file = '.'.join(vcf_file.split('.')[:-1])+'.fa'
    daf_file, div_file = '.'.join(vcf_file.split('.')[:-1])+'_2fold.daf', '.'.join(vcf_file.split('.')[:-1])+'_2fold.div'
    mkt_output_file = conf_file + '_imkt.out'
    
    with open(conf_file+'.out', 'r') as f:
        f.readline()
        slim_seed = f.readline().strip()
    
    with open('vcf2fasta.out', 'w') as output:
        subprocess.run(["python", "vcf2fasta.py", vcf_file, conf_file+'.out'], stdout=output)
    ...
    simulation_runs = '{}_params.txt'.format(conf_file)
